...
export default function LoginPage() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    if (signInError) { setError("E-Mail oder Passwort falsch."); setLoading(false); return }
    ...
    <main className="flex items-center justify-center min-h-screen px-4 grid-bg">
      <div className="fixed inset-0 pointer-events-none">
        <div className="absolute bottom-0 left-0 w-[400px] h-[400px] rounded-full"
          style={{ background: "radial-gradient(circle, rgba(255,70,85,0.05) 0%, transparent 70%)", transform: "translate(-20%, 20%)" }} />
      </div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-sm" style={{ color: "var(--muted)" }}>Willkommen zurück</p>
        </div>
        ...
        <p className="text-center text-sm mt-6" style={{ color: "var(--muted)" }}>
          Noch kein Account?{" "}
          <Link href="/auth/register" style={{ color: "var(--accent)" }} className="font-semibold">Registrieren</Link>
        </p>
      </motion.div>
    </main>
  )
